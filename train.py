...
# before starting, check whether the weights already exists
if os.path.isfile('model/model.h5'):
    response = input(
        'Model already trained. Check the model directory. Override? (y/n) ')
    if response == 'n':
        exit()

if os.path.isfile('model/model.json'):
    response = input(
        'Model already trained. Check the model directory. Override? (y/n) ')
    if response == 'n':
        exit()
